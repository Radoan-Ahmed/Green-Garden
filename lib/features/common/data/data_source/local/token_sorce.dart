...
class TokenSourceImpl implements TokenSource {
  ...
  @override
  String getTokenExpTime() {
    return _preference.getString(TokenSource.kTokenExpTimeKey) ??
        "NO_TOKEN_EXP_TIME";
  }
  ...
  Future<bool> saveTokenExpTime(int tokenExpTime) {
    return _preference.setString(
        TokenSource.kTokenExpTimeKey, tokenExpTime.toString());
  }

  @override
  String getMfiid() {
    try {
      final userProfileInfo = _preference.getString("user_info_token");
      if (userProfileInfo == null) throw 'Mfi id not found';
      return json.decode(userProfileInfo)['mfiId'];
    } catch (e) {
      throw 'Mfi id not found';
    }
  }
}
